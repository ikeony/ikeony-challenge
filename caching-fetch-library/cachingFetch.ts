...
export const useCachingFetch: UseCachingFetch = (url) => {
  ...
  const getAPIData = async () => {
    if (globalThis.myCache) {setData(globalThis.myCache)}
    else {
      try {
        const apiResponse = await fetch(url);
        const jsonResponse = await apiResponse.json();
        setData(jsonResponse);
        globalThis.myCache = jsonResponse;
      }
      catch(err) {
        setError(err);
      }
  ...
  //   axios.get(url)
  //   .then((response) => {
  //     globalThis.myCache = response.data;
 ...
 *
 */
// export const serializeCache = (): string => 'hello';
export const serializeCache = (): string => {
  return globalThis.myCache as string;

};

export const initializeCache = (serializedCache: string): void => {
  // globalThis.myCache = serializeCache;
  window.__INITIAL_DATA__ = serializedCache;
};

export const wipeCache = (): void => {
  globalThis.myCache = null;
};
